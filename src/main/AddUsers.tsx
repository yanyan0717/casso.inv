...
import { UserPlus, X, Save, Trash2, Search, Settings2, Eye, EyeOff, Check } from 'lucide-react';
...
export default function AddUser() {
  ...
  const fetchPasswordRequests = async () => {
    ...
    try {
      const querySnapshot = await getDocs(query(collection(db, 'profiles'), where('password_change_status', '==', 'pending')));
      const data = querySnapshot.docs.map(docSnap => {
        ...
        return {
          ...
          requested_password: p.requested_password || '',
          status: p.password_change_status || '',
          created_at: p.password_change_requested_at || ''
        } as PasswordRequest;
      });
  ...
  const handleApprovePassword = async (request: PasswordRequest) => {
    setActionProcessingId(request.id);
    try {
      await updateDoc(doc(db, 'profiles', request.user_id), {
        password_change_status: 'approved'
      });

      showToast('Password change request approved successfully. It will be applied when the user next accesses the app.', 'success');
      fetchPasswordRequests();
    } catch (error: any) {
  ...
  const handleRejectPassword = async (request: PasswordRequest) => {
    setActionProcessingId(request.id);
    try {
      await updateDoc(doc(db, 'profiles', request.user_id), {
        password_change_status: 'rejected',
        requested_password: ''
      });

      showToast('Password change request rejected', 'success');
      fetchPasswordRequests();
    } catch (error: any) {
  ...
  return (
    <div className="flex flex-col space-y-4 relative w-full max-w-full pb-8">
      ...
      {/* Tab Switcher */}
      <div className="flex gap-2 border-b border-gray-200 mt-2 shrink-0">
        <button
          onClick={() => setActiveTab('users')}
          className={`pb-3 px-4 text-sm font-bold transition-all relative cursor-pointer ${
            activeTab === 'users' ? 'text-[#166534]' : 'text-gray-400 hover:text-gray-600'
          }`}
        >
          All Users
          {activeTab === 'users' && (
        ...
        <button
          ...
          className={`pb-3 px-4 text-sm font-bold transition-all relative cursor-pointer ${
            activeTab === 'requests' ? 'text-[#166534]' : 'text-gray-400 hover:text-gray-600'
          }`}
        >
          Password Requests
        ...
        <div className="bg-white rounded-md shadow-sm border border-gray-200 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#f8fafc] border-b border-gray-200">
                  <th className="px-6 py-3 text-[11px] font-bold text-[#166534] uppercase tracking-wider">Date Requested</th>
                  <th className="px-6 py-3 text-[11px] font-bold text-[#166534] uppercase tracking-wider">Username</th>
                  <th className="px-6 py-3 text-[11px] font-bold text-[#166534] uppercase tracking-wider">Full Name</th>
                  <th className="px-6 py-3 text-[11px] font-bold text-[#166534] uppercase tracking-wider">Email</th>
                  <th className="px-6 py-3 text-[11px] font-bold text-[#166534] uppercase tracking-wider text-center">Actions</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {loadingRequests ? (
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-8 text-center text-gray-500">No password requests found</td>
                  </tr>
                ) : (
                  passwordRequests.map((req, index) => (
                    <tr key={req.id} className={`hover:bg-slate-50 transition-colors group border-b border-slate-100 last:border-0 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                      <td className="px-6 py-3 text-sm text-slate-500 whitespace-nowrap">
                        {new Date(req.created_at).toLocaleDateString()} {new Date(req.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                      </td>
                      <td className="px-6 py-3 text-sm text-slate-800">{req.username || '-'}</td>
                      <td className="px-6 py-3 text-sm text-slate-800">{req.full_name || '-'}</td>
                      <td className="px-6 py-3 text-sm text-slate-600">{req.email || '-'}</td>
                      <td className="px-6 py-3 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            onClick={() => handleApprovePassword(req)}
                            disabled={actionProcessingId !== null}
                            className="flex items-center justify-center w-8 h-8 rounded-full bg-emerald-50 text-emerald-600 hover:bg-emerald-100 transition-all cursor-pointer disabled:opacity-50"
                            title="Approve"
                          >
                            <Check className="w-4 h-4" />
                          </button>
                          <button
                            onClick={() => handleRejectPassword(req)}
                            disabled={actionProcessingId !== null}
                            className="flex items-center justify-center w-8 h-8 rounded-full bg-red-50 text-red-600 hover:bg-red-100 transition-all cursor-pointer disabled:opacity-50"
                            title="Reject"
                          >
                            <X className="w-4 h-4" />
                          </button>
            ...
            <form onSubmit={handleUpdate} className="p-6">
              <div className="space-y-4">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm text-blue-700">
                  ...
                  <p className="text-xs">Password changes must be done by the user through their profile settings.</p>
                </div>
              </div>
